import { CameraIcon, MapPinIcon } from "@heroicons/react/20/solid";
  ...
  return (
    <>
      ...
      {hostelList?.map((hostel) => (
        <div
          key={hostel.id}
          className="flex items-center justify-center h-fit rounded mx-1 dark:bg-gray-800"
        >
          <div className="relative w-full bg-white border border-gray-200 rounded-3xl dark:bg-gray-800 dark:border-gray-700 hover:scale-up">
            <div className="absolute left-4 top-4 flex items-center bg-[#484d49] rounded-3xl p-2">
              <p className="text-base text-white dark:text-white mx-1">
                Recently
              </p>
              <CameraIcon className="w-5 h-5 text-white dark:text-white mx-1" />
              <p className="text-base text-white dark:text-white mx-1">1/5</p>
            </div>
            {/* <button
              className="absolute right-4 top-4 inline-block text-gray-500 dark:text-gray-400 dark:hover:bg-gray-700 rounded-lg text-sm p-1.5"
              type="button"
              onClick={handleHeartClick}
            >
              <svg
                viewBox="0 0 24 24"
                fill={isHeartPink ? "pink" : "black"}
