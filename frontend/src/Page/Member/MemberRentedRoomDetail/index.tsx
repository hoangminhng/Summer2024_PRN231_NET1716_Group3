...
const MemberRoomDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchContractDetails = async () => {
      try {
        if (token && contractID) {
          const response = await getContractDetail(parseInt(contractID), token);
          //console.log(response);
          if (response != null) {
            setRoomID(response.roomID);
            console.log(roomID);
            setContractRoom(response);
            console.log(response.roomServiceDetails);
          }
        }
      } catch (error) {
          ...
          <div className="w-full md:w-1/2 space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {contractRoom.roomServiceDetails ? (
                  contractRoom.roomServiceDetails
                  ...
                  .map((service) => (
                    ...
                    >
                      ...
                      <p className="text-gray-700">
                        Price: {service.servicePrice.toLocaleString("de-DE")}{" "}
                        VNĐ
                      </p>
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-800 mb-4">
                ...
                <div>
                  {contractRoom.contractMemberDetails ? (
                    <Table
                      columns={columns}
                      dataSource={contractRoom.contractMemberDetails}
                      rowKey="contractMemberID"
                    />
                  ) : (
                    <p>Loading members...</p>
                  )}
