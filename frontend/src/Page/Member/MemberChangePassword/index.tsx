...
import { getMemberOldPassword, getMemberProfle, updateMemberPassword } from "../../../api/Owner/ownerProfile";
...
const MemberChangePassword : React.FC = () => {
    ...
    const fetchUpdatePassword = async (account: AccountPassword) => {
        try {
            if (token != undefined) {
                let data = await updateMemberPassword(account, token);
                return data;
            }
        } catch (error: any) {
    ...
    const fetchGetOldPassword = async (account: AccountPassword) => {
        try {
            if (token != undefined) {
                let data = await getMemberOldPassword(account, token);
                return data;
            }
        } catch (error: any) {
            setErrorContent(error.message);
